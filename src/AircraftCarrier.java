...
public class AircraftCarrier extends Ship {
	...
	public AircraftCarrier(char orientation, int x, int y){
		this.direction = orientation;
		
		if(orientation == 'S'){
			this.shipInfo = new Tuple[]{new Tuple(0, x, y), new Tuple(0, x, y+1), new Tuple(0, x, y+2), new Tuple(0, x, y+3)};
		}
		if(orientation == 'E'){
			this.shipInfo = new Tuple[]{new Tuple(0, x, y), new Tuple(0, x+1, y), new Tuple(0, x+2, y), new Tuple(0, x+3, y)};
		}
		if(orientation == 'N'){
			this.shipInfo = new Tuple[]{new Tuple(0, x, y), new Tuple(0, x, y-1), new Tuple(0, x, y-2), new Tuple(0, x, y-3)};
		}
